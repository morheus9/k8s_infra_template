...
# Creating a NAT gateway for outgoing traffic
resource "yandex_vpc_gateway" "nat_gateway" {
  name = "k8s-nat-gateway"

  shared_egress_gateway {} # Gateway type for outgoing traffic
}

# Creating a NAT routing table
resource "yandex_vpc_route_table" "nat_route" {
  name       = "k8s-nat-route"
  network_id = yandex_vpc_network.k8s_network.id

  static_route {
    destination_prefix = "0.0.0.0/0" # All outgoing traffic
    gateway_id         = yandex_vpc_gateway.nat_gateway.id
  }
}

# Creating subnets in different zones
resource "yandex_vpc_subnet" "k8s_subnets" {
  ...
  route_table_id = yandex_vpc_route_table.nat_route.id // Linking routes
}

# If you need a separate subnet (for example, for a node), we will create it using the same network_id:
...
output "k8s_network_id" {
  description = "ID of the created Kubernetes network"
